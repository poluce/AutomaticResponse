...
SidebarOptionsButton::SidebarOptionsButton(QWidget* parent, int index, int minimumWidth, int fixedHeight)
    : QRadioButton { parent }, icon_label { new QLabel(this) }, text_label { new QLabel(this) }, index_ { index }
{
...
void SidebarOptionsButton::setDrawPromptLineEnable(bool enable)
{
    ...
    setIconPosition(enable ? 0.6 : 0.9);
}
...
void SidebarOptionsButton::setIconPosition(float scale_factor)
{
    if (scale_factor > 1.0 || scale_factor < 0) {
        return;
    }

    this->icon_label->resize(size() * scale_factor);

    double margin_ratio = (1.0 - scale_factor) / 2.0;
    this->icon_label->move(width() * margin_ratio, height() * margin_ratio);
}
