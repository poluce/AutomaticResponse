...
Sidebar::Sidebar(QWidget* parent, int initialWidth)
    : QWidget { parent }, animation { new QPropertyAnimation(this, "size") }, verticalLayout { new QVBoxLayout(this) }
{
    ...
    this->endSize.rwidth() = this->initialSize.rwidth() + 100;
    this->endSize.rheight() = this->initialSize.rheight();
...
#else
    // Qt5 使用 Qt::FindChildOption 枚举
    auto children = this->findChildren<QWidget*>(QString(), Qt::FindDirectChildrenOnly);
